---
name: log-issue
description: Log one or more issues found during validation into the current task's issues folder. Creates individual issue-XXX.md files inside <task-folder>/issues/. Use when validation agents (architecture-validator, security-auditor) report critical or high findings that need to be tracked.
---
...
Run `git branch --show-current`. Branch name follows `task/TASK-XXX-short-description`. Task folder is `.claude/tasks/TASK-XXX-short-description/`.
...
```bash
ls .claude/tasks/<task-folder>/issues/ 2>/dev/null
```
...
For each issue to log, create `.claude/tasks/<task-folder>/issues/issue-NNN.md` with this exact format:
...
Type labels: `arquitetura`, `segurança`, `typescript`, `padrão`
...
```
Issues registradas em `.claude/tasks/<task-folder>/issues/`:
