---
name: resolve-issues
description: Resolve open issues in a task's issues folder. TRIGGER on any of these: "fix issues", "resolve issues", "temos issues na task", "novas issues na task", "issues da task", "issues abertas", "corrija as issues", any message mentioning a task name alongside "issue(s)". Looks in `.claude/tasks/<task>/issues/` for `issue-NNN.md` files (open) vs `issue-NNN.done.md` (resolved). Delegates fixing to the software-engineer agent, then renames resolved issue files.
---
...
Accept either a full path, a folder name such as `TASK-001-auth-email-otp`, or an unambiguous partial task name like "task 001" or "001".

Resolve it under `.claude/tasks/` unless the user supplied another path.
...
```bash
ls .claude/tasks/<task-folder>/issues/
```
...
- The **paths** to each open issue file (e.g. `.claude/tasks/TASK-XXX/issues/issue-001.md`) — the agent reads them directly; do not copy the content here
...
- Explicit instructions for how to act:
  ...
  - For behavioral issues: add or update a focused failing test before implementing
  - For architecture/import/dead-code issues where runtime behavior is unchanged: validate with `npm run build` or `npm run lint`
...
```bash
mv .claude/tasks/<task-folder>/issues/issue-NNN.md \
   .claude/tasks/<task-folder>/issues/issue-NNN.done.md
```
